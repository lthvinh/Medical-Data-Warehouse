...
def main():
    ...
    dim_default_procedures__df = spark.sql('''
        SELECT
            ...
            , CAST(0 AS LONG) AS Procedure_Code
            , 'Unknown' AS Procedure_Description
            , CAST(0.0 AS DOUBLE) AS Procedure_Base_Cost
    ''')
    ...
    dim_procedures__df = (
        ...
        .select(
            F.ifnull(F.col('CODE'), F.lit(0)).alias('Procedure_Code')
            , F.ifnull(F.col('DESCRIPTION'), F.lit('Unknown')).alias('Procedure_Description')
            , F.ifnull(F.col('BASE_COST'), F.lit(0.0)).alias('Procedure_Base_Cost')
        )
        .dropDuplicates()
        .select(
            F.sha2(
        ...
        .unionAll(dim_default_procedures__df)
        .orderBy(F.col('Procedure_Code').asc())
    )
    
    dim_procedures__is_existing = DeltaTable.isDeltaTable(spark, dim_procedures__full_path)
    if not dim_procedures__is_existing:
        dim_procedures__df.write.mode('overwrite').format('delta').option('path', dim_procedures__full_path).saveAsTable(dim_procedures__table_name)
    else:
        dim_procedures__delta_table = DeltaTable.forName(spark, dim_procedures__table_name)
        (
            dim_procedures__delta_table.alias('target').merge(
                source = dim_procedures__df.alias('source')
                , condition = (
