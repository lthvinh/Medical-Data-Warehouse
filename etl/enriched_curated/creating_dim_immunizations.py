...
def main():
    ...
    # DIM IMMUNIZATION GROUP--------------------------------------------
    dim_immunization_group__df = (
    ...
    dim_immunization_group__is_existing = DeltaTable.isDeltaTable(spark, dim_immunization_group__full_path)
    if not dim_immunization_group__is_existing:
        dim_immunization_group__df.write.mode('overwrite').format('delta').option('path', dim_immunization_group__full_path).saveAsTable(dim_immunization_group__table_name)
    else:
        dim_immunization_group__delta_table = DeltaTable.forName(spark, dim_immunization_group__table_name)
        (
            dim_immunization_group__delta_table.alias('target').merge(
                source = dim_immunization_group__df.alias('source')
                , condition = (
                    (F.col('source.Immunization_Group_Key') == F.col('target.Immunization_Group_Key'))
                )
            )
